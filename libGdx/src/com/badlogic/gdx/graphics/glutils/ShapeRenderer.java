...
public class ShapeRenderer implements Disposable {
	...
	/** Draws a five-pointed star using {@link ShapeType#Line} or {@link ShapeType#Filled}. */
	public void star(float x, float y, float radius, int segments) {
		if (segments != 5) throw new IllegalArgumentException("A star requires exactly 5 segments.");

		// The inner and outer radii of the star.
		float innerRadius = radius / 2;

		// Angle between each point of the star.
		float angle = MathUtils.PI / 5;

		// Store the points of the star.
		...
		for (int i = 0; i < 5; i++) {
			outerPointsX[i] = x + MathUtils.cos(i * 2 * angle) * radius;
			outerPointsY[i] = y + MathUtils.sin(i * 2 * angle) * radius;

			innerPointsX[i] = x + MathUtils.cos(i * 2 * angle + angle) * innerRadius;
			innerPointsY[i] = y + MathUtils.sin(i * 2 * angle + angle) * innerRadius;
		}

		// Draw the star's lines.
		if (shapeType == ShapeType.Line) {
			check(ShapeType.Line, ShapeType.Filled, 10);  // 5 points * 2 segments + 2 for the last connection
			for (int i = 0; i < 5; i++) {
				renderer.color(color.toFloatBits());
				renderer.vertex(outerPointsX[i], outerPointsY[i], 0); // Outer point
				renderer.vertex(innerPointsX[i], innerPointsY[i], 0); // Inner point
				renderer.vertex(innerPointsX[(i + 1) % 5], innerPointsY[(i + 1) % 5], 0); // Next inner point
				renderer.vertex(outerPointsX[(i + 1) % 5], outerPointsY[(i + 1) % 5], 0); // Next outer point
			}
		} else {
			check(ShapeType.Line, ShapeType.Filled, 10);
			for (int i = 0; i < 5; i++) {
				renderer.color(color.toFloatBits());
				renderer.vertex(x, y, 0); // Center point of the star
				renderer.vertex(outerPointsX[i], outerPointsY[i], 0); // Outer point
				renderer.vertex(innerPointsX[i], innerPointsY[i], 0); // Inner point
			}
		}
