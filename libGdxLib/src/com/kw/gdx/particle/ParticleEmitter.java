...
public class ParticleEmitter {
    ...
    private boolean noAddParticle;

    public void setNoAddParticle(boolean noAddParticle) {
        this.noAddParticle = noAddParticle;
    }

    public boolean isNoAddParticle() {
        return noAddParticle;
    }
        ...
        for (int i = 0, n = active.length; i < n; i++) {
            if (active[i]) {
                ...
                if (updateParticle(particle, delta, deltaMillis)) {
                    particle.setAlpha(pa * particle.getColor().a);
                    particle.draw(batch);
                }else {
                    active[i] = false;
                    activeCount--;
